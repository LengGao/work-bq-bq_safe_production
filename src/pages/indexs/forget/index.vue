<template>
  <view class="forget">
    <view class="form">
      ...
      <view class="input-box">
        ...
        <uni-easyinput class="input" v-model="phone" placeholder="请输入手机号码" @focus="onFocus" @blur="onBlur"
                       placeholderStyle="font-size: 28rpx; color: #ccc;" :inputBorder="false" :clearable="false">
        </uni-easyinput>
      </view>
      ...
      <view class="input-box">
        ...
        <uni-easyinput class="input" v-model="password" placeholder="请设置6至20位新密码" @focus="onFocus" @blur="onBlur"
      ...
      <view class="input-box">
        <uni-icons customPrefix="iconfont" type="icon-mima" size="36rpx" color="#ccc" style="margin-left: 10rpx" />
        <uni-easyinput class="input" v-model="passwordRepeat" placeholder="请再次确认新密码" @focus="onFocus" @blur="onBlur"
                       placeholderStyle="font-size: 28rpx; color: #ccc;" type="password" :inputBorder="false"
                       passwordIcon></uni-easyinput>
      </view>
    ...
    <view class="footer" :style="visibility ? 'visibility: hidden;' : 'visibility: visible;'">
      <text class="go-login" @click="goLogin">登录账号</text>
      <text class="bdi">|</text>
  ...
  methods: {
    ...
    validator(param) {
      if (!/^1[3,4,5,7,8,9]\d{9}$/.test(param.phone)) {
        ...
        return false
      } else if (/^\w{6,20}&/.test(param.password)) {
        uni.showToast({ icon: 'error', title: '请输入6至20位数字与字母的密码' })
        return false
